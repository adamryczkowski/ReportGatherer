...
test_that("Simplest document", {
  doc<-doc_Document$new(author = 'Ja',  title = 'test', format = 'md')
...
#  debugonce(doc$insert_paragraph)
  doc$insert_paragraph(text = "Ala ma kota")
  tab<-tibble::tibble("Nazwa"=c("Tata", "Mama", "Dziecko 1", "Dziecko 2"), Wiek=1:4)
...
test_that("Make documents from pieces", {
  doc<-doc_Standalone_Chapter$new()
  ...
  rozdz<-doc$insert_section(text = "Rozdział 1", tags = "szczegóły", chart_foldername = "podkatalog", cache_foldername = "podkatalog")
  rozdz$insert_paragraph("Tekst rozdziału 1")
  #  debugonce(rozdz$insert_section)
